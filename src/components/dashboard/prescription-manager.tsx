...
export function PrescriptionManager() {
  ...
  const { data: prescriptions, isLoading: loading } = useCollection<Prescription>(prescriptionsQuery);
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
        ...
        if(lastTakenTimestamp) {
            dataToSave.lastTaken = lastTakenTimestamp;
        }


        if (editingPrescription) {
            const docRef = doc(firestore, `users/${user.uid}/prescriptions`, editingPrescription.id);
            await updateDoc(docRef, dataToSave);
            toast({ title: 'Success', description: 'Prescription updated.' });
        } else {
            await addDoc(collection(firestore, `users/${user.uid}/prescriptions`), dataToSave);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
        await deleteDoc(doc(firestore, `users/${user.uid}/prescriptions`, id));
  ...
  const handleTakeNow = async (prescription: Prescription) => {
    if (!user || !firestore) return;
    try {
        const docRef = doc(firestore, `users/${user.uid}/prescriptions`, prescription.id);
